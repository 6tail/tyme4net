...
namespace tyme.rabbyung
{
    ...
    public class RabByungMonth : AbstractTyme
    {
        ...
        /// 藏历年
        /// </summary>
        public RabByungYear RabByungYear { get; }
        ...
        public RabByungMonth(RabByungYear year, int month)
        {
            ...
            if (y < 1950 || y > 2050)
            {
                throw new ArgumentException($"rab-byung year {y} must between 1950 and 2050");
            }

            var m = Math.Abs(month);
            if (y == 1950 && m < 12)
            {
                throw new ArgumentException($"month ${month} must be 12 in rab-byung year {y}");
            }

            var leap = month < 0;
            var leapMonth = year.LeapMonth;
            if (leap && m != leapMonth)
            {
                throw new ArgumentException($"illegal leap month {m} in rab-byung year {y}");
            }
        ...
        /// <param name="year">藏历年</param>
        /// <param name="month">月</param>
        public RabByungMonth(int year, int month) : this(RabByungYear.FromYear(year), month)
        {
        }
        ...
        /// <param name="month">月</param>
        public RabByungMonth(int rabByungIndex, RabByungElement element, Zodiac zodiac, int month) : this(RabByungYear.FromElementZodiac(rabByungIndex, element, zodiac), month)
        {
        }
        ...
        /// 年
        /// </summary>
        public int Year => RabByungYear.Year;
        ...
        /// </summary>
        /// <returns>别名</returns>
        public string Alias => (IsLeap ? "闰" : "") + Aliases[Month - 1];
        ...
        {
            return RabByungYear + GetName();
        }

        /// <summary>
        /// 推移
        /// </summary>
        /// <param name="n">推移月数</param>
        /// <returns>推移后的藏历月</returns>
        public new RabByungMonth Next(int n)
        {
            if (n == 0)
            {
        ...
        /// 首日
        /// </summary>
        public RabByungDay FirstDay => new RabByungDay(this, 1);

        /// <summary>
        /// 当月天数
        /// </summary>
        public int DayCount => 30 + LeapDays.Count - MissDays.Count;

        /// <summary>
        /// 本月的藏历日列表
        /// </summary>
        public List<RabByungDay> Days
        {
            get
            {
        ...
        public List<int> SpecialDays
        {
            ...
            {
                var l = new List<int>();
                if (Cache.TryGetValue(Year * 13 + IndexInYear, out var days))
                {
                    l.AddRange(days);
                }
        ...
        /// 闰日列表
        /// </summary>
        public List<int> LeapDays => SpecialDays.Where(d => d > 0).ToList();

        /// <summary>
        /// 缺日列表
        /// </summary>
        public List<int> MissDays => (from d in SpecialDays where d < 0 select -d).ToList();
    }
}