...
namespace tyme.lunar
{
    ...
    public class LunarYear : AbstractTyme
    {
        ...
        /// 缓存{闰月:年}
        /// </summary>
        protected static Dictionary<int, List<int>> Leap { get; } = new Dictionary<int, List<int>>();
        ...
        static LunarYear()
        {
            ...
            for (var i = 0; i < 12; i++)
            {
                ...
                {
                    var z = y * 2;
                    var s = m.Substring(z, 2);
                    var t = 0;
                    var c = 1;
                    for (var x = 1; x > -1; x--)
                    {
                        t += c * chars.IndexOf(s[x]);
                        c *= 64;
                    }
        ...
        public new LunarYear Next(int n)
        {
        ...
        /// 首月（农历月，即一月，俗称正月）
        /// </summary>
        public LunarMonth FirstMonth => LunarMonth.FromYm(Year, 1);

        /// <summary>
        /// 月份列表，一般有12个月，当年有闰月时，有13个月。
        /// </summary>
        public List<LunarMonth> Months
        {
            get
            {
